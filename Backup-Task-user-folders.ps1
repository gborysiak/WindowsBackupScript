...
function Zip_Folder($ZipFilePath, $Pwd )
{
  ...
  try
  {
    # 150310 GRBOFR Changed creation method (start)
    [System.IO.Compression.ZipFile]::CreateFromDirectory($Pwd, $ZipFilePath, $Compression, $false);
    [GC]::Collect()
...
else
{
  ...
  if ($Compress)
  {
    ...
    {		
      ...
      if ( Test-Path $BackupTempFolder )
      {
        # 150313 GRBOFR Removed native zip compression (start)
        #$zipname = "$BackupDirectory\" + $Config.CONFIG.GENERAL.BackupName  + "_" + (Get-Date -Format "yyyyMMdd") + ".zip"
        #Zip_Folder $zipname $BackupTempFolder
        if (Compress-Backup $BackupTempFolder)
        {
        # 150313 GRBOFR Removed native zip compression (finish)        
          if ( Test-Path $BackupTempFolder )
          {
            Remove-Item -Path $BackupTempFolder -Recurse -Force -Confirm:$false -ErrorAction SilentlyContinue | Out-Null
          }

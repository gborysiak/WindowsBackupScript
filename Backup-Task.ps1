...
function Get-BackupSize($File)
{
	...
	if ( $FileSummary[-1] -notlike "*error*" )
	{
		try
		{
			...
			Write-log -Message ($($Messages.get_Item(12)) -f $TotalSize) -Indent $script:Indent -Path $script:LogFile
			Return $TotalSize
		}
		catch
		{
	...
	{
		Write-log -Message "7Zip error: $($FileSummary[-1])" -Indent $script:Indent -Path $script:LogFile -Level "Error"
		[int] $SpaceNeeded = $script:Config.CONFIG.GENERAL.MininumSpaceInGB
		Write-log -Message ($($Messages.get_Item(13)) -f $SpaceNeeded) -Indent $script:Indent -Path $script:LogFile -Level "Warning"
		$JobResult.Message += $Messages.get_Item(13) -f $SpaceNeeded
		Return $SpaceNeeded
...
Write-Host "RB 2"
	...
	#$backup_letter = Split-Path $HomeBkpDir -Qualifier
	Write-Host "emplacement backup " . $HomeBkpDir
...
Write-Host "RB 3"
	...
	try
	{
Write-Host "RB 4 $WBPolicy"

		Start-WBBackup -Policy $WBPolicy -Force
	}
	catch
	{
Write-Host "RB error $($_.Exception.Message)"
